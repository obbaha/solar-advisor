...
- Only files matching these patterns are included: resources/views/**, app/Http/Controllers/**, app/Services/**, routes/**
...
if ($step < 5) {
    ...
    public function export(Request $request)
    {
        ...
        // مثال: return (new SolarExport($calculation, $input))->download("solar-calculations.{$format}");
...
class SolarCalculationService
{
    ...
    private const CABLE_CURRENT_CAPACITY = [
        ...
        'battery_dc' => 5,   // 5 أمبير لكل مم² لكابلات البطاريات
        'ac_output' => 1.5,  // 1.5 أمبير لكل مم² للتيار المتردد
    ];
    ...
    private const BATTERY_OVERSIZE_FACTOR = 2.00;    // زيادة حجم البطارية بنسبة %100
    ...
    public function calculate(array $input): array
    {
        ...
        if ($isSplitSystem) {
            ...
            $cables = $this->calculateCables($panels['total_kw'], $batteries['total_ah'], $floors);
    ...
    private function calculateSplitSystem(
        ...
        for ($i = 1; $i <= $totalBatteries; $i++) {
            ...
            $cables = $this->calculateCables($panelsPerSystem['total_kw'], $batteriesPerSystem['total_ah'], $floors);
 ...
 * @param float $batteryCapacityAh سعة البطاريات (أمبير-ساعة)
 * @param int $floors عدد الطوابق
 * 
 * @return array تفاصيل الكابلات المطلوبة
 */
private function calculateCables(float $solarPowerKw, float $batteryCapacityAh, int $floors): array
{
    // حساب التيار في دوائر الألواح الشمسية
    $solarCurrent = ($solarPowerKw * 1000) / self::SYSTEM_VOLTAGE;
    
    // حساب تيار البطارية (أقصى تيار تفريغ)
    $maxBatteryCurrent = ($solarPowerKw * 1000) / self::SYSTEM_VOLTAGE;
    
    // حساب تيار الخرج AC (بناءً على قدرة الإنفرتر)
    $acCurrent = (($solarPowerKw * 1000) * 0.8) / 220;
                                ...
                                <label for="pattern_day" 
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-gray-600 text-sm mb-2">
                                            معظم الاستهلاك خلال النهار (8 صباحاً - 6 مساءً)
                                        </p>
                            ...
                            <div class="pattern-option">
                                ...
                                <label for="pattern_night" 
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-gray-600 text-sm mb-2">
                                            معظم الاستهلاك خلال الليل (6 مساءً - 8 صباحاً)
                                        </p>
                                        <div class="flex flex-wrap gap-1">
